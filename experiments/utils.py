...
class AtariCeller(gym.Wrapper):
    ...
    def __init__(self, env: gym.Env, width: int = 11, height: int = 8, nb_pixels: int = 8) -> None:
        ...
        self.cell_space = spaces.MultiDiscrete(np.ones((height * width,), dtype=np.int64) * nb_pixels)

    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        obs, reward, done, info = super().step(action)
        cell = cv2.resize(obs, (self.width, self.height))
        cell = cv2.cvtColor(cell, cv2.COLOR_RGB2GRAY)
        cell = np.floor(cell / 255 * self.nb_pixels).astype(np.uint8).flatten()
        info["cell"] = cell
        return obs, reward, done, info
...
class GoalLogger(BaseCallback):
    ...
    def _on_step(self):
        if self.n_calls % self.freq == 0:
            ...
            goal_trajectories = [
                (traj.reshape((traj.shape[0], 8, 11, 1)) / 8 * 255).astype(np.uint64) for traj in goal_trajectories
            ]
